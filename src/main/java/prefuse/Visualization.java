...
/**
 * <p>
 * Central data structure representing an interactive Visualization. This class
 * is responsible for managing the mappings between source data and onscreen
 * VisualItems, maintaining a list of {@link Display} instances responsible for
 * rendering of and interaction with the contents of this visualization, and
 * providing a collection of named Action instances for performing data
 ...
 * <em>visual abstractions</em> of input data. Regardless of the data structure
 * (i.e., {@link prefuse.data.Table}, {@link prefuse.data.Graph}, or
 * {@link prefuse.data.Tree}), this class takes source data such as that loaded
 * from a file (see {@link prefuse.data.io}) or from a relational database (see
 * {@link prefuse.data.io.sql}) and creates a visual representation of the data.
 * These visual representations of the data are data sets in their own right,
 * providing access to the underlying source data to be visualized while also
 * adding addition data fields specific to a visualization. These fields include
 * spatial location (x, y coordinates and item bounds), color (for stroke, fill,
 * and text), size, shape, and font. For a given input data set of type
 * {@link prefuse.data.Table}, {@link prefuse.data.Graph}, or or
 * {@link prefuse.data.Tree}, a corresponding instance of
 * {@link prefuse.visual.VisualTable}, {@link prefuse.visual.VisualGraph}, or
 * {@link prefuse.visual.VisualTree} is created and stored in the visualization.
 * These data types inherit the data values of the source data (and indeed,
 * manipulate it directly) while additionally providing the aforementioned
 * visual variables unique to that generated visual abstraction. Similarly, all
 * {@link prefuse.data.Tuple}, {@link prefuse.data.Node}, or
 * {@link prefuse.data.Edge} instances used to represent an entry in the source
 * data have a corresponding {@link prefuse.visual.VisualItem},
 ...
 * {@link #add(String, TupleSet)} and the other "add" methods. These methods
 * will automatically create the visual abstraction, and store it in this
 * visualization, associating it with a provided <em>data group name
 * </em>. This group name allows for queries to this visualization that consider
 * only VisualItem instances from that particular group. This is quite useful
 * when crafting {@link prefuse.action.Action} instances that process only a
 * particular group of visual data. The Visualization class provides mechanisms
 * for querying any or all groups within the visualization, using one or both of
 * the group name or a filtering {@link prefuse.data.expression.Predicate} to
 * determine the items to include (see {@link #items(Predicate)} for an
 * examples). Source data may be added multiple times to a Visualization under
 * different group names, allowing for multiple representations of the same
 * backing data.
 * </p>
 *
 * <p>
 * Additionally, the Visualization class supports VisualItem instances that are
 * not directly grounded in backing source data. Examples include
 ...
 * representation of an aggregated of other VisualItems. Methods for adding data
 * groups of these kinds include {@link #addDecorators(String, String)} and
 * {@link #addAggregates(String)}.
 * </p>
 *
 * <p>
 ...
 * <em>secondary, or focus data groups</em> that maintain additional collections
 * of the VisualItems stored in the primary groups. Examples include a set of
 * focus items (such as those that have been clicked by the user), selected
 * items (items selected by a user), or search items (all matches to a search
 * query). The exact semantics of these groups and the mechanisms by which they
 * are populated is determined by application creators, but some defaults are
 * provided. The Visualization class includes some default group names, namely
 * {@link #FOCUS_ITEMS}, {@link #SELECTED_ITEMS}, and {@link #SEARCH_ITEMS} for
 * the above mentioned tasks. By default, both the {@link #FOCUS_ITEMS},
 * {@link #SELECTED_ITEMS} focus groups are included in the Visualization,
 * represented using {@link prefuse.data.tuple.DefaultTupleSet} instances. Also,
 * some of the interactive controls provided by the {@link prefuse.controls}
 * package populate these sets by default. See
 * {@link prefuse.controls.FocusControl} for an example.
 * </p>
 *
 * <p>
 * Visualizations also maintain references to all the {@link Display} instances
 * providing interactive views of the content of this visualization.
 * {@link Display} instances registers themselves with the visualization either
 * in their constructor or through the
 * {@link Display#setVisualization(Visualization)} method, so they do not
 * otherwise need to be added manually. Displays can be configured to show all
 * or only a subset of the data in the Visualization. A filtering
 * {@link prefuse.data.expression.Predicate} can be used to control what items
 * are drawn by the displaying, including limiting the Display to particular
 * data groups (for example, using a
 * {@link prefuse.visual.expression.InGroupPredicate}). The Visualization's
 * {@link #repaint()} method will trigger a repaint on all Displays associated
 ...
 * processing on the VisualItems contained in the visualization. Using the
 * {@link #putAction(String, Action)} will add a named Action to the
 * visualization, registering the Action such that a reference to this
 * Visualization will be available within the scope of the Action's
 * {@link prefuse.action.Action#run(double)} method. Processing Actions can
 * later be invoked by name using the {@link #run(String)} method and other
 * similar methods. This functionality not only provides a convenient means of
 * organizing a Visualization-specific collection of processing Actions, it also
 * allows for a layer of indirection between an Action and its name. This allows
 * Actions to be dynamically swapped at runtime. For example, an application may
 * make a call to invoke an Action named "layout", but the actual layout
 * processing maybe be dynamically swapped by changing the Action that
 * corresponds to that name. For more information on processing Actions, see the
...
public class Visualization {
	...
	private final Map m_visual;
	private final Map m_source;
	private final Map m_focus;
	...
	// displays
	private final ArrayList m_displays;
	...
	/**
	 * Add a data set to this visualization, using the given data group name. A
	 * visual abstraction of the data will be created and registered with the
	 * visualization. An exception will be thrown if the group name is already
	 * in use.
	 *
	 * @param group
	 ...
	 */
	public synchronized VisualTupleSet add(final String group, final TupleSet data) {
	 ...
	 *            the data to visualize
	 * @param filter
	 *            a filter Predicate determining which data Tuples in the input
	 *            data set are visualized
	 * @return a visual abstraction of the input data, a VisualTupleSet instance
	 */
	public synchronized VisualTupleSet add(final String group, final TupleSet data, final Predicate filter) {
	...
	protected void addDataGroup(final String group, final VisualTupleSet ts, final TupleSet src) {
	 ...
	 * group name. This adds a group of VisualItems that do not have a backing
	 * data set, useful for creating interactive visual objects that do not
	 * represent data. An exception will be thrown if the group name is already
	 * in use.
	 *
	 * @param group
	 *            the data group name for the visualized data
	 ...
	 */
	public synchronized VisualTable addTable(final String group) {
		final VisualTable vt = new VisualTable(this, group);
	...
	/**
	 * Add an empty VisualTable to this visualization, using the given data
	 * group name and table schema. This adds a group of VisualItems that do not
	 * have a backing data set, useful for creating interactive visual objects
	 * that do not represent data. An exception will be thrown if the group name
	 * is already in use.
	 *
	 * @param group
	 *            the data group name for the visualized data
	 ...
	 * @return the added VisualTable
	 */
	public synchronized VisualTable addTable(final String group, final Schema schema) {
		final VisualTable vt = new VisualTable(this, group, schema);
		this.addDataGroup(group, vt, null);
		return vt;
	}
	 ...
	 * A visual abstraction of the data will be created and registered with the
	 * visualization. An exception will be thrown if the group name is already
	 * in use.
	 *
	 * @param group
	 ...
	 */
	public synchronized VisualTable addTable(final String group, final Table table) {
	 ...
	 * @param filter
	 *            a filter Predicate determining which data Tuples in the input
	 *            table are visualized
	 */
	public synchronized VisualTable addTable(final String group, final Table table, final Predicate filter) {
		final VisualTable vt = new VisualTable(table, this, group, filter);
	 ...
	 */
	public synchronized VisualTable addTable(final String group, final Table table, final Schema schema) {
	...
	public synchronized VisualTable addTable(final String group, final Table table, final Predicate filter,
			final Schema schema) {
		final VisualTable vt = new VisualTable(table, this, group, filter, schema);
		this.addDataGroup(group, vt, table);
		return vt;
	}

	/**
	 * Add a VisualTable to this visualization, using the table's pre-set group
	 * name. An exception will be thrown if the group name is already in use.
	 * This method allows you to insert custom implementations of VisualTable
	 * into a Visualization. It is intended for advanced users and should
	 * <b>NOT</b> be used if you do not know what you are doing. In almost all
	 * cases, one of the other add methods is preferred.
	 *
	 * @param table
	 *            the pre-built VisualTable to add
	 * @return the added VisualTable
	 */
	public synchronized VisualTable addTable(final VisualTable table) {
	...
	/**
	 * Adds a graph to this visualization, using the given data group name. A
	 * visual abstraction of the data will be created and registered with the
	 * visualization. An exception will be thrown if the group name is already
	 * in use.
	 *
	 * @param group
	 *            the data group name for the visualized graph. The nodes and
	 *            edges will be available in the "group.nodes" and "group.edges"
	 *            subgroups.
	 * @param graph
	 *            the graph to visualize
	 */
	public synchronized VisualGraph addGraph(final String group, final Graph graph) {
	 ...
	 * @param filter
	 *            a filter Predicate determining which data Tuples in the input
	 *            graph are visualized
	 */
	public synchronized VisualGraph addGraph(final String group, final Graph graph, final Predicate filter) {
	 ...
	 */
	public synchronized VisualGraph addGraph(final String group, final Graph graph, final Predicate filter,
			final Schema nodeSchema, final Schema edgeSchema) {
		this.checkGroupExists(group); // check before adding sub-tables
		...
		VisualTable nt, et;
		nt = this.addTable(ngroup, graph.getNodeTable(), filter, nodeSchema);
		et = this.addTable(egroup, graph.getEdgeTable(), filter, edgeSchema);

		final VisualGraph vg = new VisualGraph(nt, et, graph.isDirected(), graph.getNodeKeyField(),
		...
		final TupleManager ntm = new TupleManager(nt, vg, TableNodeItem.class);
		final TupleManager etm = new TupleManager(et, vg, TableEdgeItem.class);
	...
	/**
	 * Adds a tree to this visualization, using the given data group name. A
	 * visual abstraction of the data will be created and registered with the
	 * visualization. An exception will be thrown if the group name is already
	 * in use.
	 *
	 * @param group
	 *            the data group name for the visualized tree. The nodes and
	 *            edges will be available in the "group.nodes" and "group.edges"
	 *            subgroups.
	 * @param tree
	 *            the tree to visualize
	 */
	public synchronized VisualTree addTree(final String group, final Tree tree) {
	 ...
	 *            graph are visualized
	 */
	public synchronized VisualTree addTree(final String group, final Tree tree, final Predicate filter) {
	...
	public synchronized VisualTree addTree(final String group, final Tree tree, final Predicate filter,
		...
		VisualTable nt, et;
		nt = this.addTable(ngroup, tree.getNodeTable(), filter, nodeSchema);
		et = this.addTable(egroup, tree.getEdgeTable(), filter, edgeSchema);

		final VisualTree vt = new VisualTree(nt, et, tree.getNodeKeyField(), tree.getEdgeSourceField(),
				tree.getEdgeTargetField());
		vt.setVisualization(this);
		vt.setGroup(group);

		this.addDataGroup(group, vt, tree);

		final TupleManager ntm = new TupleManager(nt, vt, TableNodeItem.class);
		final TupleManager etm = new TupleManager(et, vt, TableEdgeItem.class);
		nt.setTupleManager(ntm);
		et.setTupleManager(etm);
	...
	/**
	 * Add a group of aggregates to this visualization. Aggregates are used to
	 * visually represent groups of VisualItems.
	 *
	 * @param group
	 ...
	 */
	public synchronized AggregateTable addAggregates(final String group) {
	 ...
	 * @see prefuse.visual.AggregateTable
	 */
	public synchronized AggregateTable addAggregates(final String group, final Schema schema) {
		final AggregateTable vat = new AggregateTable(this, group, schema);
	...
	/**
	 * Add a derived table, a VisualTable that is cascaded from an existing
	 * VisualTable. This is useful for creating VisualItems that inherit a set
	 * of visual properties from another group of VisualItems. This might be
	 * used, for example, in the creation of small multiples where only a few
	 * visual attributes vary across the multiples.
	 *
	 * @param group
	 ...
	 * @param filter
	 *            a Predicate filter indicating which tuples of the source group
	 *            should be inheritable by the new group
	 * @param override
	 *            a data schema indicating which data fields should not be
	 *            inherited, but managed locally by the derived group
	 * @return the derived VisualTable
	 */
	public synchronized VisualTable addDerivedTable(final String group, final String source, final Predicate filter,
			final Schema override) {
		final VisualTable src = (VisualTable) this.getGroup(source);
		final VisualTable vt = new VisualTable(src, this, group, filter, override);
	 ...
	 * are VisualItem instances intended to "decorate" another VisualItem, such
	 * as providing a label or dedicated interactive control, and are realizeed
	 * as {@link prefuse.visual.DecoratorItem} instances that provide access to
	 * the decorated item in addition to the standard VisualItem properties. The
	 * generated table is created using the
	 * {@link #addDerivedTable(String, String, Predicate, Schema)} method, but
	 * with no VisualItem properties inherited from the source group.
	 *
	 * @param group
	 ...
	 */
	public synchronized VisualTable addDecorators(final String group, final String source) {
	 ...
	 * as providing a label or dedicated interactive control, and are realizeed
	 * as {@link prefuse.visual.DecoratorItem} instances that provide access to
	 * the decorated item in addition to the standard VisualItem properties.
	 *
	 * @param group
	 *            the data group to use for the decorators
	 * @param source
	 *            the source data group to decorate
	 * @param schema
	 *            schema indicating which variables should <b>not</b> be
	 *            inherited from the source data group and instead be managed
	 *            locally by the generated VisualTable
	 * @return the generated VisualTable of DecoratorItem instances
	 */
	public synchronized VisualTable addDecorators(final String group, final String source, final Schema schema) {
	 ...
	 * @return the generated VisualTable of DecoratorItem instances
	 */
	public synchronized VisualTable addDecorators(final String group, final String source, final Predicate filter) {
		final VisualTable t = this.addDerivedTable(group, source, filter, VisualItem.SCHEMA);
	...
	public synchronized VisualTable addDecorators(final String group, final String source, final Predicate filter,
			final Schema schema) {
		final VisualTable t = this.addDerivedTable(group, source, filter, schema);
		t.setTupleManager(new TupleManager(t, null, TableDecoratorItem.class));
		return t;
	}
	 ...
	 * will be removed, and any members of the group will also be removed from
	 * any registered focus groups.
	 *
	 * @param group
	...
	public synchronized boolean removeGroup(final String group) {
		...
		if (ts != null) {
			...
			for (final Iterator items = ts.tuples(ValidatedPredicate.TRUE); items.hasNext();) {
		...
		final TupleSet[] focus = new TupleSet[this.m_focus.size()];
		this.m_focus.values().toArray(focus);
		for (final Iterator items = ts.tuples(); items.hasNext();) {
		...
		if (ts instanceof CompositeTupleSet) {
			final CompositeTupleSet cts = (CompositeTupleSet) ts;
			for (final Iterator names = cts.setNames(); names.hasNext();) {
	...
	/**
	 * Reset this visualization, clearing out all visualization tuples. All data
	 * sets added using the "addXXX" methods will be removed from the
	...
	public synchronized void reset() {
		...
		final Iterator iter = this.m_focus.entrySet().iterator();
		while (iter.hasNext()) {
			final Map.Entry entry = (Map.Entry) iter.next();
			final TupleSet ts = (TupleSet) entry.getValue();
			ts.clear();
		}
	 ...
	 */
	public TupleSet getSourceData(final String group) {
	 ...
	 * @return the backing source data set, or null if there is no such data set
	 */
	public TupleSet getSourceData(final VisualTupleSet ts) {
	...
	public Tuple getSourceTuple(final VisualItem item) {
		...
		final TupleSet source = this.getSourceData(group);
		if (source == null) {
			return null;
		}
		...
		if (cgroup != null) {
			...
			final Graph g = (Graph) this.getSourceData(pgroup);
			if (t == g.getNodeTable()) {
	 ...
	 *
	 * @param group
	 *            the data group from which to lookup the source tuple, only
	...
	public VisualItem getVisualItem(final String group, final Tuple t) {
		final TupleSet ts = this.getVisualGroup(group);
		VisualTable vt;
	 ...
	 */
	public TupleSet getGroup(final String group) {
	...
	/**
	 * Indicates if a given VisualItem is contained in the given visual data
	 * group.
	 *
	 * @param item
	...
	public boolean isInGroup(final VisualItem item, final String group) {
		...
		final TupleSet tset = this.getGroup(group);
		return (tset == null ? false : tset.containsTuple(item));
	}

	/**
	 * Add a new secondary, or focus, group to this visualization. By default
	 * the added group is an instance of
	 ...
	 */
	public void addFocusGroup(final String group, final TupleSet tset) {
		this.checkGroupExists(group);
	 ...
	 */
	public TupleSet getVisualGroup(final String group) {
		return (TupleSet) this.m_visual.get(group);
	}

	/**
	 * Retrieve the focus data group of the given group name. Only secondary, or
	 * focus, groups will be considered.
	 *
	 * @param group
	 *            the focus data group
	 * @return the requested data group, or null if not found
	 */
	public TupleSet getFocusGroup(final String group) {
		return (TupleSet) this.m_focus.get(group);
	}

	/**
	 * Invalidate the bounds of all VisualItems in the given group. This will
	 * cause the bounds to be recomputed for all items upon the next redraw.
	 *
	 * @param group
	 *            the visual data group to invalidate
	 */
	public void invalidate(final String group) {
		final Iterator items = this.items(group, ValidatedPredicate.TRUE);
		while (items.hasNext()) {
			final VisualItem item = (VisualItem) items.next();
			item.setValidated(false);
		}
	...
	/**
	 * Invalidate the bounds of all VisualItems in this visualization. This will
	 * cause the bounds to be recomputed for all items upon the next redraw.
	 */
	 ...
	 */
	public Iterator visibleItems(final String group) {
	 ...
	 *
	 * @param filter
	 *            a Predicate indicating which items should be included in the
	 *            iteration
	 * @return a filtered iterator over VisualItems
	 */
	public Iterator items(final Predicate filter) {
		final int size = this.m_visual.size();
		if (size == 0) {
			return Collections.EMPTY_LIST.iterator();
		} else if (size == 1) {
			final Iterator it = this.m_visual.keySet().iterator();
			return this.items((String) it.next(), filter);
		} else {
			final CompositeIterator iter = new CompositeIterator(this.m_visual.size());
			final Iterator it = this.m_visual.keySet().iterator();
	 ...
	 */
	public Iterator items(final String group) {
	 ...
	 *            indicating which items should be included in the iteration.
	 *            The input string will be parsed using the
	 *            {@link prefuse.data.expression.parser.ExpressionParser} class.
	 *            If a parse error occurs, an empty iterator is returned.
	 * @return a filtered iterator over VisualItems
	 */
	public Iterator items(final String group, final String expr) {
	 ...
	 * @return a filtered iterator over VisualItems
	 */
	public Iterator items(final String group, final Predicate filter) {
		if (ALL_ITEMS.equals(group)) {
			return this.items(filter);
		}

		final TupleSet t = this.getGroup(group);
	...
	public void setValue(final String group, final Predicate p, final String field, final Object val) {
		final Iterator items = this.items(group, p);
		while (items.hasNext()) {
			final VisualItem item = (VisualItem) items.next();
			item.set(field, val);
		}
	...
	/**
	 * Sets the visbility status for all items in a given data group matching a
	 * given filter predicate.
	 *
	 * @param group
	 *            the visual data group name
	 ...
	 * visualization reference held by the removed Action will be set to null.
	 * <br/>
	 ...
	 * responsibility of clients to avoid this situation.
	 *
	 * @param name
	 *            the name of the Action
	...
	public Action removeAction(final String name) {
		...
		final Action a = this.getAction(name);
		if (a != null) {
			a.cancel();
	 ...
	 * of all Actions is managed by the {@link prefuse.activity.ActivityManager}
	 * , which runs in a dedicated thread.
	 *
	 * @param action
	 ...
	 * The running of all Actions is managed by the
	 ...
	 *            the name of the Action to run
	 * @param delay
	 *            the amount of time to wait, in milliseconds, before running
	 *            the Action
	 * @return the Action scheduled to run
	 */
	...
	/**
	 * Schedule the Action with the given name to run at the specified time. The
	 * running of all Actions is managed by the
	 * {@link prefuse.activity.ActivityManager}, which runs in a dedicated
	 * thread.
	 *
	 * @param action
	 *            the name of the Action to run
	 * @param startTime
	 *            the absolute system time, in milliseconds since the epoch, at
	 *            which to run the Action.
	 * @return the Action scheduled to run
	 */
	 ...
	 * managed by the {@link prefuse.activity.ActivityManager}, which runs in a
	 * dedicated thread.
	 *
	 * @param before
	...
	/**
	 * Schedule the Action with the given name to always run after another
	 * Action finishes running. The running of all Actions is managed by the
	 * {@link prefuse.activity.ActivityManager}, which runs in a dedicated
	 * thread.
	 *
	 * @param before
	 ...
	 * is responsible for providing the Renderer instances used to draw the
	 * VisualItems.
	 *
	 * @param rf
	 ...
	 */
	public Bounds getBounds(final String group) {
		Bounds result = new BoundingBox(0, 0, 0, 0);
		final Iterator iter = this.visibleItems(group);
		if (iter.hasNext()) {
			final VisualItem item = (VisualItem) iter.next();
			final Bounds itemBounds = item.getBounds();
			result = new BoundingBox(itemBounds.getMinX(), itemBounds.getMinY(), itemBounds.getMaxX(),
					itemBounds.getMaxY());
		}
		while (iter.hasNext()) {
			final VisualItem item = (VisualItem) iter.next();
			result = this.unionBounds(result, item.getBounds());
		}
		return result;
	}

	private Bounds unionBounds(final Bounds src1, final Bounds src2) {
		...
		final Bounds result = new BoundingBox(x1, y1, x2, y2);
		return result;
	}
	// ------------------------------------------------------------------------
	// Displays
	 ...
	 */
	public void damageReport(final VisualItem item, final Bounds region) {
		for (int i = 0; i < this.m_displays.size(); ++i) {
			final Display d = this.getDisplay(i);
