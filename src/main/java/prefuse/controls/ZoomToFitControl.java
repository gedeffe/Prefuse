...
public class ZoomToFitControl extends ControlAdapter {
	 ...
	 */
	public ZoomToFitControl(final MouseButton button) {
	 ...
	 *            {@link Control#RIGHT_MOUSE_BUTTON}.
	 */
	public ZoomToFitControl(final String group, final MouseButton button) {
		this.m_group = group;
		this.m_button = button;
	}
	 ...
	 *            {@link Control#RIGHT_MOUSE_BUTTON}.
	 */
	public ZoomToFitControl(final String group, final int margin, final long duration, final MouseButton button) {
		this.m_group = group;
	...
	public void mouseClicked(final MouseEvent e) {
		...
		if (!display.isTranformInProgress() && UILib.isButtonPressed(e, this.m_button)) {
			final Visualization vis = display.getVisualization();
			final Bounds tempBounds = vis.getBounds(this.m_group);
			Rectangle2D bounds = new Rectangle2D(tempBounds.getMinX(), tempBounds.getMinY(), tempBounds.getWidth(),
					tempBounds.getHeight());
			bounds = GraphicsLib.expand(bounds, this.m_margin + (int) (1 / display.getScale()));
