...
public class ColorLib {
	...
	public static int interp(final int c1, final int c2, final double frac) {
		...
		return rgba((int) Math.round((frac * red(c2)) + (ifrac * red(c1))),
	...
	public static int darker(final int c) {
		return rgba(Math.max(0, (int) (scale * red(c))), Math.max(0, (int) (scale * green(c))),
				Math.max(0, (int) (scale * blue(c))), alpha(c));
	}
	...
	public static int brighter(final int c) {
		...
		return rgba(Math.min(255, (int) (r / scale)), Math.min(255, (int) (g / scale)),
				Math.min(255, (int) (b / scale)), alpha(c));
	}
	...
	public static int[] getCategoryPalette(final int size, final float s1, final float s2, final float b,
		...
		float s = s1;
		for (int i = 0; i < size; i++) {
			final int j = i % CATEGORY_HUES.length;
			if (j == 0) {
