...
public class StringLib {
	...
	/**
	 * Given an array object, create a String showing the contents of the array
	 * using a "[a[0], a[1], ..., a[a.length-1]]" format.
	 *
	 * @param a
	 *            the array object
	 * @return the array string
	 */
	public static final String getArrayString(final Object a) {
		final StringBuffer sbuf = new StringBuffer();
		sbuf.append('[');
		final int size = Array.getLength(a);
	 ...
	 */
	public static String formatNumber(final double number, final int decimalPlaces) {
		final String s = String.valueOf(number);
		final int idx1 = s.indexOf('.');
		if (idx1 == -1) {
			return s;
		} else {
			final int idx2 = s.indexOf('E');
			final int dp = decimalPlaces + (idx2 >= 0 ? 0 : 1);
	...
	/**
	 * Capitalize all letters preceded by whitespace, and lower case all other
	 * letters.
	 *
	 * @param s
	...
	private static Hashtable<String, String> prefixSuffixT = new Hashtable<String, String>();
	 ...
	 */
	public static String abbreviate(final String str, final Font font, final int width) {
		int lastblank = 0, nchars = 0, cumx = 0;
		while ((cumx < width) && (nchars < str.length())) {
			if (Character.isWhitespace(str.charAt(nchars))) {
				lastblank = nchars;
			}
			cumx += computeCharWidth(str.charAt(nchars), font);
			nchars++;
		}
	 ...
	 * by this provided instance.
	 *
	 * @param string
	 *            element to analyze.
	 * @param font
	 *            specific font to use to compute string width.
	 * @return truncated width of String (translation from float to int).
	 */
	public static int computeStringWidth(final String string, final Font font) {
		return (int) Toolkit.getToolkit().getFontLoader().computeStringWidth(string, font);
	}

	/**
	 * Computes the width of the character when rendered with the font
	 * represented by this provided instance.
	 *
	 * @param character
	 *            element to analyze.
	 * @param font
	 *            specific font to use to compute character width.
	 * @return truncated width of character (translation from float to int).
	 */
	public static int computeCharWidth(final char character, final Font font) {
		final String temp = String.valueOf(character);
		return (int) Toolkit.getToolkit().getFontLoader().computeStringWidth(temp, font);
	}
	 ...
	 * @return an abbreviated String
	 */
	public static String abbreviateName(String str, final Font font, final int width) {
		if (computeStringWidth(str, font) > width) {
	 ...
	 *            the String to abbreviate
	 * @param lastOnly
	 *            true to include only the last name, false otherwise
	 * @return an abbreviated name
	 */
	private static String abbreviateName(final String inString, final boolean lastOnly) {
		final StringReader in = new StringReader(inString);
		final StreamTokenizer p = new StreamTokenizer(in);
		p.wordChars('&', '&');
		p.wordChars('@', '@');
		p.wordChars(':', ':');
		p.ordinaryChar(',');
